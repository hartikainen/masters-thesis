...
% The bottleneck application, \emph{application 1}, is shown as the upper path and the modified application, \emph{application 2}, as the lower path in figure~\ref{fig:exp1-software}, respectively.
...
% The software model is presented in figure~\ref{fig:exp1-sofware}. The high level software model consists of the input, process, and output phases. The 'packet input' and 'packet output' nodes are modeled exactly as discussed in the section~\ref{sec:characteristic-measurements}, delaying the the packets according to the equation~\ref{eq:1}.
...
% The workload model consists of a single packet stream, which is generated from a two level workload model, presented at the top in figure 7.2. The TRAFFIC GENERATOR node triggers its child node with interval RNS random uniform(5∗10−5,15∗10−5) seconds, and lifetime of 0.05 seconds. The child node creates 512B packets with interval 5.1 ∗ 10−8 ∗ RNS random lognormal(−10 seconds for 4 ∗ 10−5 seconds.
