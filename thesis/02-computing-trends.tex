...
% Today's network functionality is becoming increasingly sophisticated: data loss protection, application acceleration, intrusion detection. New services: video, mobility support services. $->$ Ever increasing need for programmable and extensible network equipment.~\cite{Egi:2009:PP}
...
% Optimizing packet processing performance of the software routers would allow developers to build and program networks using commodity hardware platforms and operating systems. The software networks promise great extensibility as the data and control plane functionality can be extended through software, mitigating the hardware design and development burden.

% A typical switch or router consists of four main components: the input ports, switch fabric, routing processor and output ports.~\cite{Kurose:2012:CNT}

% The routing and switching systems are typically built Application-Specific Integrated Circuits (ASICs), Network Processing Units (NPUs) and commodity x86 hardware.
