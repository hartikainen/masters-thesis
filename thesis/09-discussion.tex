...
As we have shown in this work, the resource network methodology, extended with customized queue disciplines and support for modeling parallel systems, can be used to model more modern MPSoC based network processing units. \todo[]{task/event parallelism, generally hardware scheduled manycoresystems?} The flexibility of PSE, and resource networks, enable simulation of large-scale systems on many different parts of the cloud computing context. However, there are certain limitations with the tool as well.
