...
The resource queues can have various scheduling disciplines, such as first in first out (FIFO), last in first out (LIFO), priority, or shortest task first.\todo{add more stuff about queue disciplines} The queuing network models are often solved analytically or simulated, to find the equilibrium or transient state of the system. \todo{explain the pros and cons of different solutions, emphasize the strict queue disciplines for analytical solutions}~\cite{Bolch:2006:Queuing}
